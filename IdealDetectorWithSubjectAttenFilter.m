...
for iSub = 1:3
    for iExp  = 1:11
        ...
        if ~isempty(subjData_mcmc(iSub).experiment(iExp).SummaryStats_precue)
            ...
            meanErrorPlusSTD = subjData_mcmc(iSub).experiment(iExp).precue.meanerror(:,1)+numSD*subjData_mcmc(iSub).experiment(iExp).precue.StandardError(:,1);
            meanErrorMinusSTD = subjData_mcmc(iSub).experiment(iExp).precue.meanerror(:,1)-numSD*subjData_mcmc(iSub).experiment(iExp).precue.StandardError(:,1);
            ...
            DiffPlus{iSub,iExp,2} = subjData_mcmc(iSub).experiment(iExp).precue.subjEff.eff - numofTargetItemsPlus;
            DiffMinus{iSub,iExp,2} = -subjData_mcmc(iSub).experiment(iExp).precue.subjEff.eff+numofTargetItemsMinus;
            ...
            meanItems(iExp,iSub,2) = mean(subjData_mcmc(iSub).experiment(iExp).precue.subjEff.eff);
        ...
        if ~isempty(subjData_mcmc(iSub).experiment(iExp).SummaryStats_postcue)
            ...
            meanErrorPlusSTD = subjData_mcmc(iSub).experiment(iExp).postcue.meanerror(:,1)+numSD*subjData_mcmc(iSub).experiment(iExp).postcue.StandardError(:,1);
            meanErrorMinusSTD = subjData_mcmc(iSub).experiment(iExp).postcue.meanerror(:,1)-numSD*subjData_mcmc(iSub).experiment(iExp).postcue.StandardError(:,1);
            ...
            DiffPlus{iSub,iExp,3} = subjData_mcmc(iSub).experiment(iExp).postcue.subjEff.eff - numofTargetItemsPlus;
            DiffMinus{iSub,iExp,3} = -subjData_mcmc(iSub).experiment(iExp).postcue.subjEff.eff+numofTargetItemsMinus;
            ...
            meanItems(iExp,iSub,3) = mean(subjData_mcmc(iSub).experiment(iExp).postcue.subjEff.eff);
        end
        ...
        if isfield(subjData_mcmc(iSub).experiment(iExp),'allCen') && ~isempty(subjData_mcmc(iSub).experiment(iExp).SummaryStats_Onetarget)
            ...
            meanErrorPlusSTD = subjData_mcmc(iSub).experiment(iExp).allCen.meanerror(1)+numSD*subjData_mcmc(iSub).experiment(iExp).allCen.StandardError(1);
            meanErrorMinusSTD = subjData_mcmc(iSub).experiment(iExp).allCen.meanerror(1)-numSD*subjData_mcmc(iSub).experiment(iExp).allCen.StandardError(1);
            ...
            DiffPlus{iSub,iExp,4} = subjData_mcmc(iSub).experiment(iExp).allCen.subjEff.eff - numofTargetItemsPlus;
            DiffMinus{iSub,iExp,4} = -subjData_mcmc(iSub).experiment(iExp).allCen.subjEff.eff+numofTargetItemsMinus;
            ...
            meanItems(iExp,iSub,4) = subjData_mcmc(iSub).experiment(iExp).allCen.subjEff.eff;
        ...
        if ~isempty(subjData_mcmc(iSub).experiment(iExp).SummaryStats_Onetarget)
            ...
            meanErrorPlusSTD = subjData_mcmc(iSub).experiment(iExp).Onetarget.meanerror(:,1)+numSD*subjData_mcmc(iSub).experiment(iExp).Onetarget.StandardError(:,1);
            meanErrorMinusSTD = subjData_mcmc(iSub).experiment(iExp).Onetarget.meanerror(:,1)-numSD*subjData_mcmc(iSub).experiment(iExp).Onetarget.StandardError(:,1);
            ...
            [~,idx_noDis] = min(abs(meanCurve_Onetarget-subjData_mcmc(iSub).experiment(iExp).Onetarget.meanerror(1:numColors)));
...
% 
% for  iType = 1:4
% h(iType)= errorbar(g1+interval(iType),meanItems_avg(g1,iType),upLimit(g1,iType),lowerLimit(g1,iType),'color',color(iType,:)); hold on
% errorbar(g2+interval(iType),meanItems_avg(g2,iType),upLimit(g2,iType),lowerLimit(g2,iType),'color',color(iType,:)); hold on  
% end
% 
% xlim([0,12])
% ylim([0,32])
% 
% legend([h(1),h(2),h(3),h(4)],'zero-distractor','pre-cued','postcue')
% 
% xticks(1:11)
% t = {'C3S0D10 NR \newline    3';'C3S0D10 \newline    3';'C4S0D8 \newline    4';'C4S0D6 \newline    4';'C2S2D6 \newline    4';'C0S4D6 \newline    4';...
%     'C5S0D6 \newline    5' ;'C6S0D5 \newline    6 ';'C3S3D5 \newline    6';'C4S3D4 \newline    7';'C4S4D4 \newline    8'};
...
title('Uncorrected subjects'' attention filters')
