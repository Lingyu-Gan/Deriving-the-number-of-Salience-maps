...
for iSub = 1:3
    for iExp  = 1:11
        ...
        if ~isempty(subjData_mcmc(iSub).experiment(iExp).SummaryStats_precue)
            ...
            meanCurve_precue = subjData_mcmc(iSub).experiment(iExp).precue.subjEff.meanCurve_perfect;
            ...
            meanItems(iExp,iSub,2) = mean(subjData_mcmc(iSub).experiment(iExp).precue.subjEff.effPerfect);
        ...
        if ~isempty(subjData_mcmc(iSub).experiment(iExp).SummaryStats_postcue)
           ...
           meanCurve_postcue = subjData_mcmc(iSub).experiment(iExp).postcue.subjEff.meanCurve_perfect;
            ...
            [~,idx_postcue] = min(abs(meanCurve_postcue-subjData_mcmc(iSub).experiment(iExp).postcue.meanerror(:,1)'));
        ...
        if isfield(subjData_mcmc(iSub).experiment(iExp),'allCen') && ~isempty(subjData_mcmc(iSub).experiment(iExp).SummaryStats_Onetarget)
            ...
            meanCurve_allcen =  subjData_mcmc(iSub).experiment(iExp).allCen.perfectEff.meanCurve;
            ...
            DiffPlus{iSub,iExp,4} = subjData_mcmc(iSub).experiment(iExp).allCen.perfectEff.eff - numofTargetItemsPlus;
            DiffMinus{iSub,iExp,4} = -subjData_mcmc(iSub).experiment(iExp).allCen.perfectEff.eff+numofTargetItemsMinus;
            ...
            meanItems(iExp,iSub,4) = subjData_mcmc(iSub).experiment(iExp).allCen.perfectEff.eff;
...
% color = [1,0,0;0.93 0.35 0.71;0,0,1;0.11,0.42,0.11];
color = [1,0,0;0.72 0.27 1;0,0,1;0.11,0.42,0.11];
...
% ylim([0,32])
% 
% legend([h(1),h(2),h(3),h(4)],'zero-distractor','pre-cued','postcue','centroid-of-ALL')
% 
% xticks(1:11)
% t = {'C3S0D10 NR \newline    3';'C3S0D10 \newline    3';'C4S0D8 \newline    4';'C4S0D6 \newline    4';'C2S2D6 \newline    4';'C0S4D6 \newline    4';...
%     'C5S0D6 \newline    5' ;'C6S0D5 \newline    6 ';'C3S3D5 \newline    6';'C4S3D4 \newline    7';'C4S4D4 \newline    8'};
...
edgealpha = 0.5;
for iType = 2:4
    plot(g1,meanItems_avg(g1,iType),'-','color',color(iType,:),'marker','.','LineWidth',1,'markersize',8);hold on
%    if  iType == 2 || iType==3
...
%    end
    ...
    plot(g2,meanItems_avg(g2,iType),'-','color',color(iType,:),'marker','.','LineWidth',1,'markersize',8);hold on
%      if  iType == 2 || iType==3
...
set(gca,'fontsize',16,'FontName','Arial')
set(0,'DefaultLineLineWidth',1)
title('Uncorrected perfect filters')
